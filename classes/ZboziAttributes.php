...
class ZboziAttributes {
    ...
    public static function getProductAttributes($id_product, $id_lang, $id_shop, $debug = false) {
        ...
        if ($debug) {
            self::$debugData['sql'] = $sql;
            self::$debugData['combinations'] = $combinations;
        }
    ...
    class GoogleCombinations extends FilterCombinations {
        ...
        protected function filterAttributes($attributes, $googleAttributes, &$removed) {
            ...
            foreach ($attributes as $attribute) {
                if (isset($googleAttributes[$attribute[5]]) && !($googleAttributes[$attribute[5]] == '0')) {
                    $retval[] = [0 => $googleAttributes[$attribute[5]], 1 => $attribute[1], 2 => $attribute[2], 3 => $attribute[3], 4 => $attribute[4], 5 => $attribute[5]];
                } else {
                    $removed[] = [0 => $attribute[0], 1 => $attribute[1], 2 => $attribute[2], 3 => $attribute[3], 4 => $attribute[4], 5 => $attribute[5]];
                }
            }
            if (!is_array($retval)) {
                return false;
    ...
    protected function filterAttributes($attributes, $filterAttributes, &$removed) {
        ...
        foreach ($attributes as $attribute) {
            if (in_array($attribute[5], $filterAttributes)) {
                $retval[] = [0 => $attribute[0], 1 => $attribute[1], 2 => $attribute[2], 3 => $attribute[3], 4 => $attribute[4], 5 => $attribute[5]];
